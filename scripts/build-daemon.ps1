<#
.SYNOPSIS
    编译 Codex 代理守护独立进程（GuardianDaemon.exe）
.DESCRIPTION
    使用系统自带 .NET Framework 4.8 csc.exe 编译，无需安装 SDK。
    输出到项目根目录 dist\GuardianDaemon.exe。
    用法:
        .\build-daemon.ps1            # 增量编译（exe 存在且比源码新时跳过）
        .\build-daemon.ps1 -Force     # 强制重新编译
#>
[CmdletBinding()]
param([switch]$Force)
...
$src = Join-Path $root 'src\GuardianDaemon.cs'
$dist = Join-Path $root 'dist'
$out = Join-Path $dist 'GuardianDaemon.exe'
...
if ((Test-Path -LiteralPath $out) -and -not $Force) {
    ...
    if ($outTime -ge $srcTime) { "已是最新: $out"; exit 0 }
}

New-Item -ItemType Directory -Force -Path $dist | Out-Null

& $csc /nologo /target:winexe /utf8output /win32icon:"$(Join-Path $dist 'guardian.ico')" /out:"$out" `
